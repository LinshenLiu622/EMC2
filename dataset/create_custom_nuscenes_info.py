...
### the libarary I add start 
from pcdet.ops.roiaware_pool3d import roiaware_pool3d_utils
from pcdet.utils import common_utils
from pcdet.datasets.dataset import DatasetTemplate
from pyquaternion import Quaternion
from PIL import Image
### the libarary I add end
...
class NuScenesDataset(DatasetTemplate):
    ...
    def evaluation(self, det_annos, class_names, **kwargs): # 这里是官方定义的 
        import json
        from nuscenes.nuscenes import NuScenes
        from . import nuscenes_utils
        nusc = NuScenes(version=self.dataset_cfg.VERSION, dataroot=str(self.root_path), verbose=True) 
        # 初始化 NuScenes 的官方 Python SDK。 
        # load the thing such as 23 category, 8 attribute, 4 visibility, 
        nusc_annos = nuscenes_utils.transform_det_annos_to_nusc_annos(det_annos, nusc)
        # version：例如 'v1.0-trainval'、'v1.0-test'
        nusc_annos['meta'] = {
            'use_camera': False,
            'use_lidar': True,
            'use_radar': False,
...
def create_nuscenes_info(version, data_path, save_path, max_sweeps=10, with_cam=False, scene_list=None):
    
    def load_custom_scenes(scene_txt_path, max_scene_num=20):
        with open(scene_txt_path, 'r') as f:
            scenes = [line.strip() for line in f.readlines()]
        return scenes[:max_scene_num]
    ...
    custom_scene_names = load_custom_scenes(Path(scene_list), max_scene_num=20)
    train_scenes = custom_scene_names
    val_scenes = []
    else:
    ...
    if version == 'v1.0-trainval':
        ...
        # custom_scene_names = load_custom_scenes(Path('VEE_dataset.txt'), max_scene_num=20)
        # train_scenes = custom_scene_names
        # val_scenes = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_file', type=str, default=None, help='/home/admin/Desktop/lliu/openpcdet/OpenPCDet/tools/cfgs/dataset_configs/nuscenes_dataset.yaml')
    parser.add_argument('--func', type=str, default='create_nuscenes_infos', help='')
    parser.add_argument('--version', type=str, default='v1.0-trainval', help='') # v1.0-trainval, # v1.0-mini
    parser.add_argument('--with_cam', action='store_true', default=False, help='use camera or not')
    parser.add_argument('--scene_list', type=str, default=None,help='VEE_dataset.txt') # 'Path to custom scene list (e.g., VEE_dataset.txt)' 
